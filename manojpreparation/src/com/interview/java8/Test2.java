...
public class Test2 {
    ...
    public static void main(String[] args) {
     ...
     List list=   employeeList.stream()
                ...
                .map(emp->departmentList.stream().filter(dep->dep.getDepartmentId()==emp.getDepartmentId()
                ).findFirst().map(Department::getDepartmentName).orElse("")).collect(Collectors.toList());
       ...
       Map map= employeeList.stream()
                ...
                .collect(Collectors.toMap(emp->emp.getName(),emp->departmentList
                        .stream()
                        .filter(dep->dep.getDepartmentId()==emp
                                .getDepartmentId()).findFirst()
                        .map(Department::getDepartmentName).orElse("")
                ));
        System.out.println(map
        );
        // counting of employee in each department

        Map<Integer ,String> stringIntegerMap= departmentList.stream()
        ...
        Map<String,Long> stringLongMap= employeeList.stream()
                .collect(Collectors
                        .groupingBy(emp->stringIntegerMap.get(emp.getDepartmentId()),Collectors.counting()));
